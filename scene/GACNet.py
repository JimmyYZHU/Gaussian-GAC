...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False):
    ...
    if points is not None:
        ...
        fps_points = torch.cat([new_xyz, fps_points], dim=-1) # centroids with featues
        # normed pts with features
...
class GraphAttention(nn.Module):
    ...
    def forward(self, center_xyz, center_feature, grouped_xyz, grouped_feature):
        ...
        # center_xyz: sampled pts
        # center_feature: sampled pts(centroids) with feat (after mlp)
        # grouped_xyz: neighbors of the centroieds
        # grouped_feature: centralized neighbors with feat (after mlp)
        Return:
...
class GraphAttentionConvLayer(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        # grouped_xyz: neighbors of the sampled points [B, npoint, nsample, C]
        # fps_points: sampled points with feat [B, npoint, C+D,1]
        new_points = new_points.permute(0, 3, 2, 1) # [B, C+D, nsample,npoint]
        fps_points = fps_points.unsqueeze(3).permute(0, 2, 3, 1) # [B, C+D, 1,npoint]
        # TODO: why the position infomation is input to get the feature
        for i, conv in enumerate(self.mlp_convs):
            bn = self.mlp_bns[i]
            fps_points = F.relu(bn(conv(fps_points))) # sampled points with feat
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            dists, idx = dists[:, :, :3], idx[:, :, :3]  # [B, N, 3], only the three nearest neighbors
